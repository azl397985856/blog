...
注意这个代币只是模拟现实玉米价格，而不是反应 web3 上的交易价格。实际应用中，常常会将实际玉米价格作为参考值而动态调整一些数据，比如我做了一个国债的 RWA，那么质押的回报率就可以定期取真正的国家发布的国债利率，而不是固定的某个利率。另外也可以将现实中的东西（比如房产证）以 NFT 的形式抵押在智能合约中等等，这些都是 RWA 的应用，而不仅仅是本文讲的这一种形式。

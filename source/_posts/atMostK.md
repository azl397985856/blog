...
我继续修改下题目， 如果让你求一个数组相邻差为 1 连续子数组的总个数呢？（如果只有一个数，那么我们也认为其实一个相邻差为 1 连续子数组）其实就是**索引差 1 的同时，值也差 1。**

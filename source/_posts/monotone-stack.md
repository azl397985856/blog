...
由于单调栈的特性可知， 单调栈适合求解下一个更大的或者下一个更小的数。而实际上，也适合求**上一个**更大的或者更小的。

因为上一个其实就是 pop 出来后，栈顶的数。下一个其实就是导致你被 pop 出来的那个数。

知道了这一点，大家可以使用 [2104. 子数组范围和](https://leetcode-cn.com/problems/sum-of-subarray-ranges/) 练习一下。
